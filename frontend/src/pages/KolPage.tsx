...
import type { EcosystemTag, KolAccount, KolLabel } from '../lib/types';
...
export default function KolPage() {
  ...
  const [labels, setLabels] = useState<KolLabel[]>([]);
  ...
  const [form, setForm] = useState({
    x_handle: '', display_name: '', chain_ids: [] as EcosystemTag[], custom_label_ids: [] as string[], weight: 5,
  });

  const fetchLabels = useCallback(async () => {
    const response = await api.kol.labels.list();
    if (response.ok && response.data) setLabels(response.data);
    return response;
  }, []);
  ...
  const openAdd = () => {
    setEditItem(null);
    setForm({ x_handle: '', display_name: '', chain_ids: [], custom_label_ids: [], weight: 5 });
    setModalOpen(true);
  };
  ...
  const openEdit = (item: KolAccount) => {
    ...
    setForm({
      ...
      custom_label_ids: item.custom_labels?.map((label) => label.id) || [],
  ...
  const createLabel = async (name: string) => {
    const response = await api.kol.labels.create(name);
    if (!response.ok || !response.data) {
      toast(response.error || '新增标签失败', 'error');
      return null;
    }
    setLabels((current) => [...current.filter((label) => label.id !== response.data?.id), response.data as KolLabel]
  ...
  const columns = [
    ...
    { header: '所属生态', accessor: (row: KolAccount) => <div className="kol-tag-list">{row.chain_ids?.length ? row.chain_ids.map((item) => <span key={item}>{TAG_LABELS[item] || item}</span>) : <span>未分类</span>}</div> },
    { header: '自定义标签', accessor: (row: KolAccount) => { const visible = row.custom_labels?.slice(0, 3) || []; const rest = Math.max(0, (row.custom_labels?.length || 0) - visible.length); const title = row.custom_labels?.map((label) => label.name).join('、') || '无自定义标签'; return <div className="kol-tag-list custom" title={title}>{visible.length ? visible.map((label) => <span key={label.id}>{label.name}</span>) : <span>无</span>}{rest > 0 && <span>+{rest}</span>}</div>; } },
    ...
    <div className="kol-page">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setModalOpen(false)} title={editItem ? '编辑 KOL' : '添加 KOL'}>
        <div className="flex flex-col gap-md">
          ...
          <div className="flex flex-col gap-xs"><span className="text-sm text-secondary font-medium">所属生态</span><div className="kol-tag-editor">{TAG_OPTIONS.filter((item) => !['all', 'unclassified'].includes(item.value)).map((item) => <button key={item.value} type="button" className={form.chain_ids.includes(item.value as EcosystemTag) ? 'active' : ''} onClick={() => toggleTag(item.value as EcosystemTag)}>{item.label}</button>)}</div></div>
          <div className="flex flex-col gap-xs"><span className="text-sm text-secondary font-medium">自定义标签</span><KolLabelPicker labels={labels} selectedIds={form.custom_label_ids} onChange={(custom_label_ids) => setForm((current) => ({ ...current, custom_label_ids }))} onCreate={createLabel} /></div>
      ...
      <KolLabelManager open={isLabelManagerOpen} labels={labels} onClose={() => setLabelManagerOpen(false)} onChanged={async () => { await Promise.all([fetchLabels(), fetchData(false)]); }} />
      </>}
    </div>
  );
}
