...
The current release keeps the authenticated frontend gate, fixed CA trading, P19 execution,
the P20 account-scoped dynamic strategy pipeline, and the P21 follow-discovery strategy.
...
2. 只上传 Git 中的代码、Migration `000-040`、依赖锁文件和同 commit 构建产物；禁止上传 `.env`、API Key、GMGN 私钥、OPENNEWS_TOKEN、ADMIN_TOKEN、数据库密码、日志和数据库 dump。
...
4. 应用 Migration 前先由 Supervisor 的独立 Migration phase 执行 `npm run migrate`；再以生产只读方式运行 `npm run audit:schema:production`。必须确认 Migration `027` 至 `040`、动态模板、按链预算、Follow Policy/Event、候选索引、Family 拆分、共享限流、验证快照和租约字段完整。
...
冷部署必须保持 `P21_FOLLOW_DISCOVERY_ENABLED=false`。固定 CA、动态喊单、持仓和离场回归通过后，先在设置页只开启新关注发现能力，并将策略保持为 Record。确认 Follow 方向、Baseline、永久去重、Target 身份、官网 SSRF 防护、唯一 CA 和 GMGN 精确验证均符合预期后，再单独审批 Paper 或单 KOL 小额 Live。开启 P21 能力只重启 ingestion/监控进程，不得自动启动全局 Engine。
