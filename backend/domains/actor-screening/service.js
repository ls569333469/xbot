...
function parseRunId(id) {
  ...
  if (!Number.isSafeInteger(value) || value < 1) {
    const error = new Error('Screening run id must be a positive integer');
    error.code = 'ACTOR_SCREENING_ID_INVALID';
    throw error;
  }
  return value;
}
...
async function getRun(id, executor = db) {
  ...
  const result = await executor.query(
    `SELECT run.*, COALESCE(json_agg(result ORDER BY result.x_handle)
     ...
     WHERE run.id = $1 GROUP BY run.id`, [runId]
  );
...
async function retryFailedRun(id, executor = db) {
  ...
  await executor.query(
    `UPDATE x_actor_screening_runs
     ...
     WHERE id = $1 AND status <> 'cancelled'`, [runId]
  );
  return true;
}
