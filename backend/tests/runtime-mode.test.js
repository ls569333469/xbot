...
test('getTradingMode accepts production signal/live modes and isolates paper', () => {
  assert.equal(getTradingMode({ TRADING_MODE: 'signal' }), 'signal');
  assert.equal(getTradingMode({ TRADING_MODE: 'live' }), 'live');
  assert.equal(getTradingMode({ TRADING_MODE: 'PAPER', XBOT_LEGACY_PAPER_ENABLED: 'true' }), 'paper');
  assert.throws(() => getTradingMode({ TRADING_MODE: 'paper' }), { code: 'LEGACY_PAPER_DISABLED' });
...
test('assertLiveMode requires both live mode and armed state', () => {
  ...
  assert.throws(() => assertLiveMode(armed, { TRADING_MODE: 'paper' }), { code: 'LEGACY_PAPER_DISABLED' });
